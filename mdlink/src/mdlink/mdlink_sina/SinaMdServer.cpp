...
bool SinaMdServer::init_by_type(MdlinkCfg& mdcfg)
{
    if (mdcfg.route != "SINA") {
    ...
    if (mdvec.size() != 1) {
        LOG(FATAL) << "More than one sina sources are given";
        return false;
    }
    m_cfg = mdvec[0];     
    return true;    
}
bool SinaMdServer::is_same_quote(shared_ptr<sina_api::MarketQuote> md1,
...
bool SinaMdServer::sina_to_jzs(shared_ptr<sina_api::MarketQuote> md, MarketQuote* pbk)
{
    ...
    MarketQuote& bk = *pbk;
    auto it = m_sina_inst_map.find(md->code);
    if (it == m_sina_inst_map.end()) {
        return false;
    }
    ...
    for (auto it = m_maptables->g_jzcode_to_instrument.begin();
        ...
        if (insttype_set.find(inst.insttype) != insttype_set.end() && inst.mkt < mkt_size) {
            ...
            if (mkts_set.find(mkt_code) != mkts_set.end()) {
                string sina_code = sina_mktcodes[inst.mkt] + inst.instcode;
                m_sina_inst_map[sina_code] = inst;
                m_query_codes.push_back(sina_code);
            }
        }       
    }  
